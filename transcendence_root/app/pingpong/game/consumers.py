...
class PingPongGameConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        self.room_code = self.scope['url_route']['kwargs']['room_code'] #room_code abc123 from the nested.
        self.room_group_name = f"game_{self.room_code}" # group_name like game_abc123.
    ...
    async def receive(self, text_data): #here where I stopped today .
    ...
    async def send_game_state(self):
        ...
        await self.channel_layer.group_send(
            self.room_group_name,
            {
                'type': 'game_update',  #the handler ...
                'game_state': game_state, # the event passed there.
            }
        )
